	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/headbg05.jpg)'}">
		...
		<div class="h-information">
			...
			<h2 class="h-description">
                        ...
                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"追风赶月莫停留，平芜尽处是春山"}}
...
.h-description {
  ...
  font-weight: bold;
  background: -webkit-linear-gradient(rgb(28, 245, 216), rgb(255, 119, 142));
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
  text-fill-color: transparent;
  text-shadow: 2px 2px 4px rgba(255, 255, 255, 0.5); /* 添加的文本阴影效果 */
}
